...
async def get_all(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    query = db.query(Todos).filter(Todos.owner == user.get('user_id')).all()
    return query
...
async def get_by_id(id: int = Path(gt=0), user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    query = db.query(Todos).filter(Todos.id == id).filter(Todos.owner == user.get('user_id')).first()
    if query is None:
...
async def create_todo(todo: SendTodo, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    todo_to_post = Todos(**todo.model_dump(), owner=user.get('user_id'))#transform the request body into a Todo Object(open the JSON 'dictionary' and transform in an object of todo class)
    db.add(todo_to_post)
    db.commit()
...
async def edit_todo(todo: SendTodo, id: int = Path(gt=0), user: dict = Depends(get_current_user), db: Session = Depends(get_db)):#ppydantic model before id(Path parameters)
    query = db.query(Todos).filter(Todos.id == id).filter(Todos.owner == user.get('user_id'))
